/*       Data Acquisition Recording Window
...
DataAcquisitionRecordGui::DataAcquisitionRecordGui(QSerialPort* p, QWidget* parent)
                                                    : QDialog(parent)
{
    socket = p;
...
// Ask for the microcontroller SD card free space (process response later)
    ...
    DataAcquisitionRecordUi.fileTableView->setModel(model);
    DataAcquisitionRecordUi.fileTableView->setGridStyle(Qt::NoPen);
    DataAcquisitionRecordUi.fileTableView->setShowGrid(false);
    QHeaderView *verticalHeader = DataAcquisitionRecordUi.fileTableView->verticalHeader();
    verticalHeader->setSectionResizeMode(QHeaderView::Fixed);
    verticalHeader->setDefaultSectionSize(18);
    row = 0;
// Signal to process a click on a directory item
    connect(DataAcquisitionRecordUi.fileTableView,
...
void DataAcquisitionRecordGui::on_startButton_clicked()
{
    ...
    else DataAcquisitionRecordUi.errorLabel->setText("File not open");
}
...
void DataAcquisitionRecordGui::on_stopButton_clicked()
{
    socket->write("pr-\n\r");
    requestRecordingStatus();
}
...
void DataAcquisitionRecordGui::on_closeFileButton_clicked()
{
    if (writeFileHandle < 0xFF)
    {
        char command[6] = "fC0\n\r";
...
// Show Free Space
        case 'F':
        {
            ...
            DataAcquisitionRecordUi.diskSpaceAvailable->setText(QString("%1 M")\
                                                    .arg(freeSpace, 0, 10));
            break;
        }
/* Directory listing.
...
// The write and read file handles are retrieved from this
        case 's':
        {
            ...
            if (recordingOn)  // recording on
                DataAcquisitionRecordUi.startButton->
            ...
            if (writeFileOpen)
            {
                DataAcquisitionRecordUi.recordFileButton->
                    setStyleSheet("background-color:lightgreen;");
                DataAcquisitionRecordUi.recordFileName->setText(breakdown[3]);
            }
            else
                DataAcquisitionRecordUi.recordFileButton->
                    setStyleSheet("background-color:lightpink;");
            ...
            if (readFileOpen)
                 DataAcquisitionRecordUi.readFileName->setText(breakdown[3]);
           break;
        }
// Open a file for recording.
        ...
        case 'E':
        {
            ...
            if ((status > 0) && (status < 20))
                DataAcquisitionRecordUi.errorLabel->setText(errorText[status-1]);
            break;
        }
    }
}
...
int DataAcquisitionRecordGui::extractValue(const QString &response)
{
    int i = 4;
    QString temp;
    while (response[i] >= '0')
...
void DataAcquisitionRecordGui::onListItemClicked(const QModelIndex & index)
{
    DataAcquisitionRecordUi.recordFileName->clear();
    QStandardItem *item = model->itemFromIndex(index);
    QString fileName = item->text();
    QChar type = item->data().toChar();
    if (type == 'f')
    {
        DataAcquisitionRecordUi.recordFileName->setText(fileName);
        DataAcquisitionRecordUi.readFileName->setText(fileName);
    }
    if (type == 'd')
...
void DataAcquisitionRecordGui::on_closeButton_clicked()
{
    this->close();
}
...
void DataAcquisitionRecordGui::requestRecordingStatus()
{
    socket->write("fs\n\r");
}
...
void DataAcquisitionRecordGui::getFreeSpace()
{
    socket->write("fF\n\r");
}
